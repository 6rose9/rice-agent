...
function FollowingContent() {
  ...
  const loadData = useCallback(async () => {
    setLoading(true);
    try {
      const [following, connCount, followerCount, followingCount, pending] =
        await Promise.all([
          getFollowingProfiles(),
  ...
  if (!isAuthenticated) {
    return (
      <SignInGate
        ...
        description="Follow rice traders, farmers, millers, and agents to stay updated."
        redirectTo="/mynetwork/following"
      />
    );
  }
        ...
        <div className="px-4 py-4 sm:px-6 sm:py-5 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              {loading ? "Loading..." : `${profiles.length} following`}
            </p>
          </div>
        </div>
